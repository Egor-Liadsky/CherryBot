...
class Take:
    ...
    def take_group_list(self) -> list:
        ...
        if len(self.group_list) == 0:
            response = requests.post('https://www.rksi.su/')
            soup = BeautifulSoup(response.content, "html.parser")
            teachers_select = soup.findAll(['select'])[1]
            group_select = soup.findAll(['select'])[0]
            for i in group_select.findAll(['option']):
                self.group_list.append(i.text.upper())
            for i in teachers_select.findAll(['option']):
                i = i.text
    ...
    def schedule(self, cmd: str) -> Union[list, None]:
        ...
        if self.domain == 'ru':
            ...
            if is_group:
                self.resonse = requests.post('https://www.rksi.ru/schedule/',
            ...
            self.resonse = requests.post('http://rksi.su',
                                         data=data,
                                         headers=self.headers)

        soup = BeautifulSoup(self.resonse.content, "html.parser")
